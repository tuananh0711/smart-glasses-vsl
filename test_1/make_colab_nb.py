 ...
 "cells": [
  {
   ...
   "source": [
    ...
    "3. Chạy Ô Số 3 để Train Mô Hình BiGRU + Attention. Mô hình sẽ tự động lưu lại vào trong Drive của bạn."
   ]
  },
  {
   "cell_type": "code",
  ...
  },
  {
   ...
   "source": [
    ...
    "from tensorflow.keras.layers import GRU, Bidirectional, Dense, Dropout, BatchNormalization, LayerNormalization, MultiHeadAttention, Add, GlobalAveragePooling1D, GlobalMaxPooling1D, Concatenate, Input\n",
    ...
    "        yield data, label_id\n",
    "\n",
    "def get_dataset(split='train', batch_size=32, num_classes=None):\n",
    ...
    "    \n",
    "    # Làm phẳng dữ liệu cho GRU và chuyển label sang one-hot\n",
    "    def reshape_fn(x, y):\n",
    "        batch_size_dyn = tf.shape(x)[0]\n",
    "        x_reshaped = tf.reshape(x, [batch_size_dyn, MAX_FRAMES, NUM_POINTS * NUM_DIMS])\n",
    "        y_onehot = tf.one_hot(y, depth=num_classes) if num_classes else y\n",
    ...
    "    inputs = Input(shape=flattened_shape)\n",
    "    \n",
    ...
    "\n",
    "train_dataset = get_dataset(split='train', batch_size=BATCH_SIZE, num_classes=num_classes)\n",
    "val_dataset = get_dataset(split='test', batch_size=BATCH_SIZE, num_classes=num_classes)\n",
    "\n",
    "print(f\"Khởi tạo mô hình BiGRU + Attention cho {num_classes} từ vựng...\")\n",
    "model = build_bigru_attention_model((MAX_FRAMES, NUM_POINTS, NUM_DIMS), num_classes)\n",
    "\n",
    "checkpoint_path = os.path.join(MODELS_DIR, 'vsl_bigru_attention_colab.keras')\n",
    "callbacks = [\n",
...
# Lấy thư mục hiện tại của file make_colab_nb.py
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
with open(os.path.join(BASE_DIR, "Train_Colab.ipynb"), "w", encoding="utf-8") as f:
